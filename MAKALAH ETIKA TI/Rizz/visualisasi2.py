...
# Hitung kemunculan kata "rizz" per URL video
rizz_count_per_url = {}
all_words = []
...
for idx, entry in enumerate(data):
    if isinstance(entry, dict):  # Memastikan ini adalah metadata URL
        ...
        rizz_count_per_url[current_url] = 0  # Inisialisasi
    elif isinstance(entry, str):  # Komentar aktual
        # Hitung jumlah kemunculan kata 'rizz' (case insensitive)
        count = entry.lower().count('rizz')
        rizz_count_per_url[current_url] += count
        
        # Tambahkan semua kata kecuali 'rizz' dan stopwords untuk analisis WordCloud
...
# Visualisasi jumlah kemunculan 'rizz' per URL
plt.figure(figsize=(10, 5))
bars = plt.bar(
    range(len(rizz_count_per_url)),
    rizz_count_per_url.values(),
    color=['#4c72b0'] * len(rizz_count_per_url),  # Warna biru gradien terang
    edgecolor='black'
)
